...
${html}
    ...
    return (
        <div className="card" style={{ borderColor: 'rgba(16,185,129,0.3)' }}>
            ...
            {/* Preview */}
            <div className="confluence-preview">{tableHtml}</div>
            ...
            <div className="flex gap-8 mt-12">
                ...
                <button className="btn btn-primary btn-sm" onClick={handlePublish} disabled={published}>
                    {published ? <><CheckCircle size={13} /> Published!</> : <><Send size={13} /> Publish to Confluence</>}
                </button>
            </div>
            ...
            {published && (
                <div style={{ marginTop: 10, padding: '10px 14px', background: 'var(--emerald-dim)', borderRadius: 8, fontSize: 12, color: 'var(--emerald)', display: 'flex', gap: 8, alignItems: 'center' }}>
                    <CheckCircle size={14} /> Successfully published to Confluence page "Weekly Project Status" — Row added for {report.week}.
                </div>
            )}
