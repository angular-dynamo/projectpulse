...
function MilestoneRow({ milestone, totalWidth }: { milestone: Milestone; totalWidth: number }) {
    const barStart = (dayOffset(milestone.targetDate) / YEAR_DAYS) * totalWidth;
    const barWidth = 12;
    const color = STATUS_COLORS[milestone.status];
    const prevOffset = milestone.actualDate
        ? (dayOffset(milestone.actualDate) / YEAR_DAYS) * totalWidth
        : null;
...
export default function GanttChart() {
    ...
    // Today line
    const todayOffset = (differenceInDays(new Date(), YEAR_START) / YEAR_DAYS) * CHART_W;
    const projectStart = project ? (dayOffset(project.startDate) / YEAR_DAYS) * CHART_W : 0;
    const projectEnd = project ? (dayOffset(project.endDate) / YEAR_DAYS) * CHART_W : CHART_W;
                ...
                <div className="gantt-wrapper">
                    <svg width={SVG_W} height={totalH} style={{ minWidth: SVG_W }}>
                        ...
                        {MONTH_LABELS.map((m, i) => {
                            ...
                            return (
                                <g key={m}>
                                    ...
                                    <text x={x + (CHART_W / 24)} y={20} fill="var(--text-muted)" fontSize={10} textAnchor="middle">{m}</text>
                                </g>
                            );
                        })}
                        ...
                        {milestones.map((ms, i) => {
                            ...
                            const dateLabel = format(parseISO(ms.targetDate), 'MMM d');
                            return (
                                <g key={ms.id}>
                                    ...
                                    <text x={8} y={y + 34} fill="var(--text-muted)" fontSize={9} style={{ fontFamily: 'Inter,sans-serif' }}>
                                        {dateLabel}{ms.actualDate ? ` → actual: ${format(parseISO(ms.actualDate), 'MMM d')}` : ''}
                                    </text>
                                    ...
                                    {/* Date label */}
                                    <text x={LABEL_W + (dayOffset(ms.targetDate) / YEAR_DAYS) * CHART_W + 10}
